...
{
    int i, j, k;
    printf("Before calloc of s which is size: %d\n", sizeof(sortable_bbox) * total);
    ///////////////// Ok this calloc is overrunning the heap, only prints when running gdb though
    sortable_bbox *s = (sortable_bbox*) calloc(total, sizeof(sortable_bbox));
    if (!s)
    {
        printf("calloc of s failed.\n");
        return;
    }
    else if (!boxes | !probs)
    {
        printf("boxes or probs null\n");
        return;
    }
    else
    {
